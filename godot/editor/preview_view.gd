...
func set_gizmos(value: VFXViewportGizmos) -> void:
	gizmos = value
...
func frame_aabb(bounds: AABB, margin: float = 1.35) -> void:
	if preview_camera == null:
		return
	target = bounds.get_center()
	var radius: float = max(0.35, max(bounds.size.x, max(bounds.size.y, bounds.size.z)) * 0.5 * margin)
	var fov_radians: float = deg_to_rad(preview_camera.fov * 0.5)
	distance = clamp(radius / max(0.01, tan(fov_radians)) * 1.15, 2.5, 24.0)
	yaw = 0.68
	pitch = 0.38
	_apply_orbit()
...
func _gui_input(event: InputEvent) -> void:
	if preview_camera == null:
		return
	if gizmos != null and event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		if event.pressed and gizmos.try_begin_drag(preview_camera, event.position):
			_gizmo_dragging = true
			drag_mode = 0
			accept_event()
			return
		elif not event.pressed and _gizmo_dragging:
			gizmos.finish_drag()
			_gizmo_dragging = false
			accept_event()
			return
	if _gizmo_dragging and event is InputEventMouseMotion:
		gizmos.update_drag(preview_camera, event.position)
		accept_event()
		return
